...
__version__ = "0.0.8-hacked-for-only-rst"
...
def find_files_in_dir(path_to_dir):
    return list(Path(path_to_dir).rglob('*.rst'))
...
def main():
    ...
    input_path_argument = args.input_path
    assert os.path.exists(input_path_argument)

    paths: List[str] = [input_path_argument] if not os.path.isdir(input_path_argument) else find_files_in_dir(input_path_argument)

    all_links_found: List[str] = []
    exceptions = {}

    for path_to_file in paths:
        with open(str(path_to_file)) as input_file:
    ...
    link_list = map(lambda el: (el, exceptions), all_links_found)
